...
class MazeEnv():
    ...
    def render(self, time = 0.0001, show = False):
    ...
    def step(self, action):
        ...
        if(self.current_position == new_position):
            ...
            self._episode_ended = True
            self.current_position = new_position
            return self.current_observation(), 50, True
        else:
            self.current_position = new_position

            distance_reward = 5 / (1 + np.linalg.norm(np.array(new_position) - np.array(self.goal)))
