...
from PathEnv_v11 import MazeEnv
...
class DQNAgent:
    ...
    def _build_model(self):
        model = Sequential()
        model.add(Conv2D(32, (5, 5), input_shape=(self.state_size, self.state_size, 1), activation='relu'))
...
def save_model(agent, episode, model_dir='models'):
    try:
        ...
        model_path = os.path.join(model_dir, f'v11_model_{episode}.keras')  # 加入情节编号
...
def are_all_elements_equal(my_deque, num = 5):
    # 如果deque是空的，直接返回True
    # print('len: ', len(my_deque))
    if(len(my_deque)!=num) or not my_deque:
        return False
...
if __name__ == '__main__':
    ...
    for episode in range(EPISODES):
        ...
        # max_step_cnt.append(step)
        # if(are_all_elements_equal(max_step_cnt, num = 10) and max_step_cnt[0] == episodes_max_step and agent.epsilon<0.6):
        #     agent.epsilon = agent.epilon_max*agent.epilon_max_decay
        ...
        if episode%200 ==0 and episode!=0:
            ...
            plt.savefig(f'v11_loss_reward_plot_episode_{episode}.png')  # Save the reward plot
