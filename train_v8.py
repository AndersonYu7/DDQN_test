...
from PathEnv_v8 import MazeEnv
...
if __name__ == '__main__':
    ...
    for episode in range(EPISODES):
        ...
        # print('cnt: ', cnt)
        print('cnt2: ', cnt2)
        are_all_eqeue = all(x == agent.rewards_stable[0] for x in agent.rewards_stable)
        if(done and agent.epsilon>agent.epsilon_min):
            print('reset 1')
            # cnt+=1
            state = env.hit_reset()
        elif(done and (agent.epsilon <= agent.epsilon_min or are_all_eqeue)):
            print('reset 2')
            # cnt = 0
            cnt2+=1
        ...
        if episode%50 ==0 and episode!=0:
            ...
            plt.savefig(f'loss_reward_plot_episode_{episode}.png')  # Save the reward plot
